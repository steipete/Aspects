...
- (void)testInsteadHook {
    ...
    NSTextField *testLabel = [NSTextField new];
    testLabel.stringValue = @"Default text";
    XCTAssertEqualObjects(testLabel.stringValue, @"Default text", @"Must match");
    id aspect = [testLabel aspect_hookSelector:@selector(stringValue) withOptions:AspectPositionInstead usingBlock:^(id instance, NSArray *arguments) {
        NSInvocation *invocation = arguments.lastObject;
        NSString *customText = @"Custom Text";
        [invocation setReturnValue:&customText];
    } error:NULL];
    XCTAssertEqualObjects(testLabel.stringValue, @"Custom Text", @"Must match");

    // Test second object, and ensure that this doesn't change the override of the first object.
    NSTextField *testLabel2 = [NSTextField new];
    testLabel2.stringValue = @"Default text2";
    XCTAssertEqualObjects(testLabel2.stringValue, @"Default text2", @"Must match");
    id aspect2 = [testLabel2 aspect_hookSelector:@selector(stringValue) withOptions:AspectPositionInstead usingBlock:^(id instance, NSArray *arguments) {
        NSInvocation *invocation = arguments.lastObject;
        NSString *customText = @"Custom Text2";
        [invocation setReturnValue:&customText];
    } error:NULL];
    XCTAssertEqualObjects(testLabel2.stringValue, @"Custom Text2", @"Must match");

    // Globally override.
    id globalAspect = [NSTextField aspect_hookSelector:@selector(stringValue) withOptions:AspectPositionInstead usingBlock:^(id instance, NSArray *arguments) {
        NSInvocation *invocation = arguments.lastObject;
    ...
    NSTextField *testLabel3 = [[NSTextField alloc] init];
    XCTAssertEqualObjects(testLabel3.stringValue, @"Global", @"Must match");
    testLabel3.stringValue = @"Test";
    XCTAssertEqualObjects(testLabel3.stringValue, @"Global", @"Must match");
    XCTAssertTrue([globalAspect remove], @"Must work");
    XCTAssertEqualObjects(testLabel3.stringValue, @"Test", @"Must match");

    // Test that removing an aspect returns the original.
    XCTAssertEqualObjects(testLabel.stringValue, @"Default text", @"Must match"); // Note: since the global aspect is removed, it removes aspects on instances with the same selector name
